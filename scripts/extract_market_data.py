...
from utils.constants import MARKET_PROFILE_PATH, PROCESSED_MARKET_IDS_PATH, PLAYER_PROFILE_PATH
...
def main() -> None:
    ...
    try:
        with ThreadPoolExecutor() as executor:
            executor.submit(extract_player_info, MARKET_PROFILE_PATH)
        time.sleep(0)
...
def extract_player_info(filename: str) -> None:
    ...
    try:     
        if os.path.exists(PROCESSED_MARKET_IDS_PATH):
            with open(PROCESSED_MARKET_IDS_PATH, 'r') as f:
                processed_ids = set(line.strip() for line in f)
        else:
            processed_ids = set()
            with open(PROCESSED_MARKET_IDS_PATH, 'w') as f:  # Create the file if it doesn't exist
                pass
     
        player_info_df = pd.read_csv(PLAYER_PROFILE_PATH)
        ...
        def process_player(session: requests.Session, player_id: str):
            ...
            try:
                ...
                with open(PROCESSED_MARKET_IDS_PATH, 'a') as f:
                    f.write(f'{player_id}\n')

                pd.DataFrame(rows).to_csv(MARKET_PROFILE_PATH, index=False, mode='a', header=not os.path.exists(MARKET_PROFILE_PATH))
        ...
        combined_df.to_csv(MARKET_PROFILE_PATH, index=False, mode='w+', header=True)
