...
import pandas as pd
import time
from utils.constants import PLAYER_PROFILE_PATH, PROCESSED_STATS_URLS_PATH, STATS_PROFILE_PATH
...
async def main():
    ...
    df_players = pd.read_csv(PLAYER_PROFILE_PATH)
    ...
    try:
        with open(PROCESSED_STATS_URLS_PATH, 'r') as f:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for i, player_id in enumerate(player_ids):
            ...
            if (i + 1) % 1000 == 0:
                ...
                df_pivot.to_csv(STATS_PROFILE_PATH, mode='w+', header=True, index=False)

                # Write processed_ids to a text file
                with open(PROCESSED_STATS_URLS_PATH, 'w+') as f:
    ...
    df = pd.DataFrame(all_data)
    df.to_csv(STATS_PROFILE_PATH, mode='w+', header=True, index=False)
